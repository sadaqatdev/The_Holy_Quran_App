...
class _SurahIndexScreenState extends State<SurahIndexScreen> {
  ...
  // List<SuraName?>? chapters = [];
  List<SuraName?>? searchedChapters = [];
  ...
  Future<void> getSuras() async {
    final chapterCubit = SurasCubit.cubit(context);

    chapterCubit.fetchSuras();
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: BlocBuilder<SurasCubit, SurasState>(builder: (context, state) {
            if (state is SurasFetchLoading) {
              return const Center(
                child: CircularProgressIndicator(),
              );
            } else if (state is SurasFetchSuccess) {
              //

              List<SuraName?>? chapters = state.data;

              print("Sura name lenght ${chapters?.length}");
                      ...
                      children: [
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton(
                            ...
                            onPressed: () {
                              // chapterCubit.fetchSuras(api: true);
                            },
